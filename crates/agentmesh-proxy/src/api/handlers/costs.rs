...
use agentmesh_store::costs::{Granularity, GroupBy};
...
pub async fn cost_summary(
    ...
    match agentmesh_store::costs::get_cost_summary(&state.pool, from, to, granularity).await {
...
pub async fn cost_breakdown(
    ...
    match agentmesh_store::costs::get_cost_breakdown(&state.pool, from, to, group_by).await {
        Ok(rows) => {
            let total = rows.len();
            (
                StatusCode::OK,

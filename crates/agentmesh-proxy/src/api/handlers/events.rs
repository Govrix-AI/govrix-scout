...
use agentmesh_store::EventFilter;
...
pub async fn list_events(
    ...
    match agentmesh_store::events::list_events(&state.pool, &filter).await {
        Ok(events) => {
...
pub async fn get_event(
    ...
    match agentmesh_store::events::get_event(&state.pool, uuid).await {
...
pub async fn get_session_events(
    ...
    match agentmesh_store::events::get_session_events(&state.pool, uuid).await {
        Ok(events) => {
            let total = events.len();
            (
                StatusCode::OK,

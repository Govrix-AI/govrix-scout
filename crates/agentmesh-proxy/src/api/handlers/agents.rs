...
use agentmesh_store::AgentFilter;
...
pub async fn list_agents(
    ...
    match agentmesh_store::agents::list_agents(&state.pool, &filter).await {
        Ok(agents) => {
...
pub async fn get_agent(
    ...
    match agentmesh_store::agents::get_agent(&state.pool, &id).await {
...
pub async fn update_agent(
    ...
    let result = agentmesh_store::agents::update_agent_metadata(
        &state.pool,
        &id,
        body.name.as_deref(),
...
pub async fn retire_agent(
    ...
    match agentmesh_store::agents::retire_agent(&state.pool, &id).await {
        Ok(_) => (
            StatusCode::OK,
            Json(json!({
